...
Current product scope is App notes and custom URL/file/folder tabs. No app-side
...
title, geometry) and an empty context on normal opening. Custom tab creation passes the
explicitly entered URL or path as context. It must return within 500 ms.
Invocation happens in a separate short-lived Python process; timed-out or
malformed providers report an error while other providers continue. Providers
...
another app. `label` is optional short UI text; `locator` is optional. Reopening
only permits declared HTTP(S) and file URI schemes and always follows
a user action. It never constructs shell fragments from provider payloads.
